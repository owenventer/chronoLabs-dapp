  ...
  async function getNFTs() {
    ...
    async function getUserNFT() {
      ...
      const getMetadata = async () => {
        ...
        data.forEach((nft, i) => {
          if (
            data[i]["onChainMetadata"]["metadata"]["data"]["creators"][0][
              "address"
            ...
            const offChainAttributes =
              data[i]["offChainMetadata"]["metadata"]["attributes"];
            ...
            if (attributeExists) {
              setNFTs((prevNFTs) => [
                ...prevNFTs,
                {
                  ...
                  pay: null,
                  ssNum: null,
                  state: attributes.find((item) => item.type === "State")
                    ?.value,
                  country: attributes.find((item) => item.type === "Country")
                    ?.value,
                  startDate: null,
                  employeeStatus: null,
                  isCollection: true,
                  businessType: attributes.find(
                    (item) => item.type === "Business Type"
                  )?.value,
                  einNumber: attributes.find(
                    (item) => item.type === "EIN Number"
                  )?.value,
              ...
              setNFTs((prevNFTs) => [
                ...prevNFTs,
                {
                  ...
                  ssNum: attributes.find(
                    (item) => item.type === "Social Security #"
                  )?.value,
                  state: attributes.find((item) => item.type === "State")
                    ?.value,
                  country: attributes.find((item) => item.type === "Country")
                    ?.value,
                  startDate: attributes.find(
                    (item) => item.type === "Start Date"
                  )?.value,
